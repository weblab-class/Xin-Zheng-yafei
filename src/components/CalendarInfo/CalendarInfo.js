...
const SortableItemAll = SortableElement(({indexNum, value, onClickAll}) => <li className={s.textWrap} onClick={()=>onClickAll(indexNum)}>{value}</li>);
const SortableItemSelected = SortableElement(({indexNum, value, onClickSelected}) => <li className={s.textWrapClick} onClick={()=>onClickSelected(indexNum)}>{value}</li>);

const SortableList = SortableContainer(({panel, all, selected, onClickAll, onClickSelected}) => {
  return (
    <div>

    <ul>
    {selected.map((value, index) =>
      <SortableItemSelected key={`item-${index}`}  value={value} index={index} collection="1" onClickSelected={onClickSelected} indexNum={index}/>
    )}
  </ul>
      <ul>
        {all.map((value, index) =>
          <SortableItemAll key={`item-${index}`}  value={value} index={index} collection="0" onClickAll={onClickAll} indexNum={index}/>
        )}
      </ul>
    </div>
  );
...
class CalendarInfo extends React.Component {
  ...
  onSortEnd({oldIndex, newIndex}){
    this.setState({
      ...this.state,
      allTitle: arrayMove(this.state.allTitle, oldIndex, newIndex)
    });
  };


  onClickAll(index){
    console.log("clickall"+index);
    const tempAll = this.state.allTitle;
    const tempSelect = this.state.selectedTitle;
  ...
  componentWillMount(){
    ...
    this.setState(
      {...this.state,
        allTitle: wordByLine.slice(1),
        selectedTitle: wordByLine.slice(0,1)
      });
  }
      ...
      <div className={s.container}>
        ...
        <div className={s.appWrap}>
          ...
          <div className={s.areaContent}>
            <SortableList panel = {this.state.currentPannel} selected={this.state.selectedTitle} all={this.state.allTitle} onSortEnd={this.onSortEnd}
                          onClickAll={this.onClickAll} onClickSelected={this.onClickSelected} pressDelay={200}/>
          </div>
